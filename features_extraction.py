    ...
    for head in soup.find_all('head'):
        for head.link in soup.find_all('link', href=True):
            ...
            if wiki in head.link['href'] or len(dots) == 1 or domain in head.link['href']:
                return 1[0, 1, 2, 3, 4, 5, 6, 8, 9, 11, 12, 13, 14, 15, 16, 17, 22, 23, 24, 25, 27, 29]
            else:
                return -1
    return 1
...
def main(url):
    # with open(LOCALHOST_PATH + 'html/Malicious-Web-Content-Detection-Using-Machine-Learning/markup.txt', 'r') as file:
